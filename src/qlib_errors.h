...
#define QLIB_STATUS_RET_CHECK(func)                                                                           \
    {                                                                                                         \
        ...
        {                                                                                                     \
            QLIB_DEBUG_PRINT(QLIB_VERBOSE_ERROR, " error [%d %s] in: " #func, ___ret, STATUS_TO_STR(___ret)); \
            return ___ret;                                                                                    \
        }                                                                                                     \
    }
...
#define QLIB_STATUS_RET_CHECK_GOTO(func, ret, exit)                                                       \
    {                                                                                                     \
        ...
        {                                                                                                 \
            QLIB_DEBUG_PRINT(QLIB_VERBOSE_ERROR, " error [%d %s] in: " #func, (ret), STATUS_TO_STR(ret)); \
            goto exit;                                                                                    \
        }                                                                                                 \
    }
...
#define QLIB_ASSERT_RET(cond, err)                                                                            \
    {                                                                                                         \
        if (!(cond))                                                                                          \
        {                                                                                                     \
            QLIB_DEBUG_PRINT(QLIB_VERBOSE_ERROR, " ASSERT [%d %s] in: %s", (err), STATUS_TO_STR(err), #cond); \
            return err;                                                                                       \
        }                                                                                                     \
    }
...
#define QLIB_ASSERT_GOTO(cond, exit)                                       \
    {                                                                      \
        if (!(cond))                                                       \
        {                                                                  \
            QLIB_DEBUG_PRINT(QLIB_VERBOSE_ERROR, " ASSERT in: %s", #cond); \
            goto exit;                                                     \
        }                                                                  \
    }
